...
# Rollout Function
def rollout_one_episode(model, env_state_cache, path_info, use_wm, wm_mode, cot_mode):
...
# Run Rollouts
save_dir = '/u/shuhan/projects/vla/data/highway_env/rollout_experiment_collision_multiple_rewind_fixed_dataset'
...
rollout_dataloader = DataLoader(rollout_dataset, batch_size=1, shuffle=False, collate_fn=collate_fn_env_init_state, num_workers=4)

rollout_count = len(rollout_dataset)
progress_bar = tqdm.tqdm(range(rollout_count))

# for rollout_idx in tqdm.tqdm(range(rollout_count)):
for env_state_cache, path_info in rollout_dataloader:
  with torch.no_grad():
    scores = rollout_one_episode(model, env_state_cache[0], path_info[0], use_wm, wm_mode, cot_mode)
